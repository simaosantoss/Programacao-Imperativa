...
float calcular_media(Produto menu[], int total) {
    int i;
    float soma = 0.0;
    for (i = 0; i < total; i++) {
        soma += menu[i].preco;
...
bool contem_ingredientes(char *ingredientes[], const char *ing1, const char *ing2) {
    bool tem_ing1 = false, tem_ing2 = false;
    int i;
    for (i = 0; i < 5; i++) {
        if (ingredientes[i] != NULL) {
            if (strcmp(ingredientes[i], ing1) == 0) tem_ing1 = true;
            if (strcmp(ingredientes[i], ing2) == 0) tem_ing2 = true;
        }
    }
    return tem_ing1 && tem_ing2;
}
...
{72, "Chá de Frutos Vermelhos", {"Chá de frutos vermelhos", "Morango", "Mel", "Água", "Açúcar"}, 1.80}
    ...
    float media = calcular_media(menu, total);
    int contador = 0;

    for ( i = 0; i < total; i++) {
        if (contem_ingredientes(menu[i].ingredientes, "Frutos secos", "Iogurte") && menu[i].preco < media) {
            contador++;
        }
    }

    printf("Número de produtos com 'Frutos secos' e 'Iogurte' e preço abaixo da média: %d\n", contador);
